...
class NFTbuilder():
    def __init__(self) -> None:
        ...
        self.cards = self.GetJson("carddata")
        self.ids, self.names = self.fetchdata()
    ...
    def createImage(self):
        ...
        beard = random.randint(0, len(next(os.walk(f"{os.getcwd()}\\layers\\beard"))[2])-1)
        ...
        id = f"{beard}.{chest}.{eyes}.{hair}.{head}.{mouth}.{nose}"
        print(id)
    ...
    def developimg(self, id):
        name = self.getRandName()
        arr = list(map(int, id.split(".")))
        sum_of_digits = sum(int(digit) for digit in arr)
        rarity = self.getrarity(sum_of_digits)
        #if sum_of_digits
        ...
        with open(f"{os.getcwd()}\output\{id}.png", "w") as f:
    ...
    def getLayer(self, name):
        with open(f"{os.getcwd()}\\layers\\{name}", 'r') as file:
            ...
            try:
                regex = r"<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" width=\"256\" height=\"256\" viewBox=\"0 0 256 256\">(.*?)</svg>"
